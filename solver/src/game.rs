...
impl fmt::Display for Action {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SaveDice(value) => write!(f, "SaveDice {value}"),
...
impl fmt::Display for TilesError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "Tiles in game must be between {TILES_MIN_NUMBER} and {TILES_MAX_NUMBER}"
        )
    }
}
...
pub fn create_valid_tiles(tiles: Vec<u8>) -> Result<Tiles, TilesError> {
    let valid_tiles: bool = !tiles.is_empty()
        && tiles.iter().min().unwrap() >= &TILES_MIN_NUMBER
        && tiles.iter().max().unwrap() <= &TILES_MAX_NUMBER;
    match valid_tiles {
        true => Ok(Arc::from(tiles.into_boxed_slice())),
...
impl GameState {
    ...
    pub fn new(
        ...
        let valid_size = hand.iter().map(|&count| count as u16).sum::<u16>()
            + dice_throw
                .unwrap_or([0; N_FACES as usize])
                .iter()
                .map(|&count| count as u16)
                .sum::<u16>()
            <= N_DICE as u16;
        match valid_size {
            true => Ok(GameState { hand, dice_throw }),
    ...
    pub fn compute_score(&self, tiles: &[u8]) -> u8 {
        ...
        if (TILES_MIN_NUMBER..=TILES_MAX_NUMBER).contains(&score) {
            ...
            return if tiles.contains(&score) {
                TILE_WORMS[(score - TILES_MIN_NUMBER) as usize]
            } else {
                0
            };
        }
...
pub fn max_expected_score(
    ...
    let result = match action {
        Action::Bust => 0.0,
        Action::Stop => state.compute_score(tiles) as f64,
        Action::SaveDice(select_die) => match &state.dice_throw {
            Some(dice_throw) => {
                ...
                new_state
                    .available_actions()
                    .map(|a| max_expected_score(&new_state, a, tiles, cache))
                    .reduce(f64::max)
                    .unwrap()
            }
            ...
            outcomes_iter
                .map(
                    ...
                    |prob_state| {
                        ...
                        let max_exp_next_score = prob_state
                            .state
                            .available_actions()
                            .map(|a| max_expected_score(&prob_state.state, a, tiles, cache))
                            .reduce(f64::max)
                            .unwrap();
...
fn create_distinct_roll_outcomes_iter(state: &GameState) -> impl Iterator<Item = ProbGameState> {
    ...
    outcome_values.map(move |outcome| {
        ...
        ProbGameState {
            prob: compute_relative_probability(&frequency_map, throw_count_factorial)
                / total_outcomes,
            state: { new_state },
        }
    })
...
fn compute_relative_probability(
    frequency_map: &[u8; N_FACES as usize],
    n_total_factorial: usize,
) -> f64 {
    n_total_factorial as f64
        / (frequency_map
            .iter()
            .map(|&count| factorial(count as usize) as f64)
            .product::<f64>())
}
...
mod tests {
    use super::*;
    const ALL_SCORE_TILES: [u8; N_TILES as usize] = [
    ...
    mod expected_score {
        ...
        fn stop_expected_score_matches_current_hand_score() {
            ...
            let exp_score = max_expected_score(
                &state,
                Action::Stop,
                &Arc::from(ALL_SCORE_TILES),
                &mut cache,
            );
        ...
        fn rolling_stronger_hand_outperforms_weaker_hand() {
            ...
            let exp_score1 = max_expected_score(
                &state1,
                Action::Roll,
                &Arc::from(ALL_SCORE_TILES),
                &mut cache,
            );
            let exp_score2 = max_expected_score(
                &state2,
                Action::Roll,
                &Arc::from(ALL_SCORE_TILES),
                &mut cache,
        ...
        fn handle_available_tiles_set_score() {
            ...
            let exp_score_no_tile = max_expected_score(
                &state,
                Action::Stop,
                &Arc::from([22, 23, 24, 33]),
                &mut cache,
            );
